...
void LoganVoiceEngineImpl::LoganVoe_UnInit()
{
	...
	if (netw)
	{
#ifdef REGISTER_TRANS
		netw->DeRegisterExternalTransport(NULL);
#endif
		netw->Release();
	}
#ifdef REGISTER_TRANS
	if (rtp_rtcp)
	{
		rtp_rtcp->DeRegisterRTCPObserver(NULL);
		rtp_rtcp->Release();
	}
#endif
...
//	int iRet = base->RegisterVoiceSendData(_sendData);
    //����VoiceChanTrans
#ifdef REGISTER_TRANS
	int iRet = rtp_rtcp->RegisterRTCPObserver(channel, _rtcpObserver);
	iRet = netw->RegisterExternalTransport(channel, _transPort);
#endif
